...
class DiscussionMagnetsController implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        try {
            ...
            // Pobierz discussionId z atrybutów requestu (Flarum 1.x)
            $discussionId = (int) $request->getAttribute('discussionId');

            // Fallback: spróbuj z query params
            if (!$discussionId) {
            ...
            foreach ($posts as $post) {
                ...
                // === Metoda 1: Szukaj tagów <MAGNET> w przechowywanym XML ===
                if (stripos($xml, '<MAGNET') !== false) {
                    ...
                    if (preg_match_all('/<MAGNET[^>]*>(.*?)<\/MAGNET>/is', $xml, $matches)) {
                        foreach ($matches[1] as $content) {
                            ...
                            $this->addMagnetToList($magnetUri, $post->id, $magnets, $seenTokens);
                        }
                    }
                ...
                // === Metoda 2: Szukaj BBCode [magnet]...[/magnet] w treści ===
                // Posty stworzone przed instalacją rozszerzenia mogą mieć surowy BBCode
                if (stripos($xml, '[magnet]') !== false) {
                    if (preg_match_all('/\[magnet\](.*?)\[\/magnet\]/is', $xml, $bbcodeMatches)) {
                        foreach ($bbcodeMatches[1] as $content) {
                            $content = trim($content);
                            if (empty($content)) {
                                continue;
                            }
    ...
    /**
     * Dodaj magnet URI do listy (unika duplikacji kodu)
     */
    private function addMagnetToList(string $magnetUri, int $postId, array &$magnets, array &$seenTokens): void
    {
