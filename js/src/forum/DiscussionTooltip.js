...
export default class DiscussionTooltip {
    ...
    setupGlobalListeners() {
        ...
        // Ukryj przy scrollu strony (bez capture - reaguj na scroll głównego kontenera)
        window.addEventListener('scroll', () => {
            this.hide();
        });
        // Flarum używa .App-content jako scroll container
    ...
    async show(discussionId, targetElement) {
        ...
        // Sprawdź cache przed pokazaniem loading - jeśli wiemy że nie ma magnetów, nie pokazuj
        const cached = this.tooltipCache.get(discussionId);
        if (cached && Date.now() - cached.timestamp < this.cacheTimeout) {
            const cachedData = cached.data;
            if (!cachedData.success || !cachedData.magnets || cachedData.magnets.length === 0) {
                return; // Nie pokazuj tooltipa dla dyskusji bez magnetów
            }
        }
